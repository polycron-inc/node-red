...
module.exports = {
     ...
     *  - userDir
     *  - nodesDir
     *  - storageModule (新增：使用後端 API 存儲)
     ******************************************************************************/

    /** 使用後端 API 存儲模組
     * 啟用此選項後，Node-RED 的流程、憑證、設定等將存儲到後端 API 伺服器
     * 而不是本地文件系統。
     */
    // storageModule: require("@node-red/runtime/lib/storage/backend-api"),

    /** 後端 API URL
     * 設定後端 API 伺服器的 URL
     * 也可以通過環境變數 NODERED_BACKEND_API_URL 設定
     */
    backendApiUrl: process.env.NODERED_BACKEND_API_URL || "https://fms-go.winshare.app/api/v1/nodered",

    /** 流程名稱
     * 用於在後端區分不同的流程配置
     * 也可以通過環境變數 NODERED_FLOW_NAME 設定
     */
    backendFlowName: process.env.NODERED_FLOW_NAME || "default",
     ...
     */
    // env-driven editor/admin auth. Set NODERED_ADMIN_USER + NODERED_ADMIN_PASSWORD
    // in the deploy's .env to lock the editor; leave them unset for open access
    // (same as upstream default). Plain-text compare is used on purpose so the
    // deploy doesn't have to bcrypt-hash secrets — the password only ever lives
    // in the container's env, never on disk.
    adminAuth: process.env.NODERED_ADMIN_USER && process.env.NODERED_ADMIN_PASSWORD ? {
        type: "credentials",
        users: function (username) {
            if (username === process.env.NODERED_ADMIN_USER) {
     ...
     */
    // Mount admin UI + API at /nodered/ so it can be served via fms-frontend
    // reverse proxy (avoids exposing port 1880 to the public internet).
    // Override via NODERED_HTTP_ADMIN_ROOT env if you need '/' for legacy setups.
    httpAdminRoot: process.env.NODERED_HTTP_ADMIN_ROOT || '/nodered',
    ...
    httpAdminMiddleware: function (req, res, next) {
        ...
        res.set('Access-Control-Allow-Headers', 'Origin, X-Requested-With, Content-Type, Accept, Authorization, x-access-token, x-auth-user');
    ...
    httpNodeCors: {
        ...
        allowedHeaders: "Origin, X-Requested-With, Content-Type, Accept, Authorization, x-access-token, x-auth-user"
    },
